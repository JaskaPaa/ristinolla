...
<button on:click={onClick} class="square square-colors" style="width:{size}px; height:{size}px;">
    ...
    {#if text === 'X'}	
        ...
        <line class:anim={anim} opacity="0.6" stroke-width="12%" x1="80%" y1="20%" x2="20%" y2="80%"></line>
    {/if}
    {#if text === 'O'}    
        <circle class:anim={anim} fill="none" opacity="0.6" stroke-width="12%" cx="50%" cy="50%" r="30%">
        </circle>
    {/if} 
    </svg>  
</button>
